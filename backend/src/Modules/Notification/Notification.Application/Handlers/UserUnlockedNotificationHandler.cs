...
internal sealed class UserUnlockedNotificationHandler(
    ILogger logger,
    NotificationOrchestrator _orchestrator
    )
    : BaseService(logger.ForContext<UserUnlockedNotificationHandler>(), null)
{
    public async Task HandleAsync(IUserUnlockedEvent evt, CancellationToken cancellationToken)
    {
        var model = new
        {
            evt.UserName,
        ...
        {
            UserId = evt.UserId,
            TemplateKey = NotificationTemplateKey.UserUnlocked,
            Model = model,
