...
internal sealed class PasswordChangedNotificationHandler(
    ...
    : BaseService(logger.ForContext<PasswordChangedNotificationHandler>(), null)
{
    public async Task HandleAsync(IPasswordChangedEvent evt, CancellationToken cancellationToken)
    {
        var model = new
        {
            evt.UserName,
