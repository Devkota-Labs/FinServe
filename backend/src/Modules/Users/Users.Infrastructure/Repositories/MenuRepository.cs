...
internal sealed class MenuRepository(UserDbContext db) : IMenuRepository
{
    ...
    {
        return await _db.Menus.AnyAsync(x => x.Id == id, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        await _db.Menus.AddAsync(Menu, cancellationToken).ConfigureAwait(false);
