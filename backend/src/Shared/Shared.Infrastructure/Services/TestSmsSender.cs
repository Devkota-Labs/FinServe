...
internal sealed class TestSmsSender(ILogger logger, IEmailSender emailSender) : BaseService(logger.ForContext<TestSmsSender>(), null), ISmsSender
{
    public async Task SendSmsAsync(string mobileNo, string message, CancellationToken cancellationToken = default)
    {
        await emailSender.SendEmailAsync(mobileNo, "Verify your Mobile Number - FinServe", message, cancellationToken).ConfigureAwait(false);
    ...
    public async Task SendSmsAsync(string name, string mobileNo, string otp, int expiryMinutes)
    {
        ...
        await emailSender.SendEmailAsync(mobileNo, "Verify your Mobile Number - FinServe", body).ConfigureAwait(false);
