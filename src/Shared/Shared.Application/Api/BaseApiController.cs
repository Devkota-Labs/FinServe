...
public abstract class BaseApiController(ILogger logger) : ControllerBase
{
    ...
    protected IActionResult Created<T>(string actionName, object route, T data) where T : class
    {
        var response = new ApiResponse<T>(HttpStatusCode.Created, null, data);
        return CreatedAtAction(actionName, route, response);
    }

    protected IActionResult Created<T>(string message, string actionName, object route, T data) where T : class
    {
        var response = new ApiResponse<T>(HttpStatusCode.Created, message, data);
        return CreatedAtAction(actionName, route, response);
    }

    protected IActionResult Created<T>(string message, T data) where T : class
    {
        var response = new ApiResponse<T>(HttpStatusCode.Created, message, data);
